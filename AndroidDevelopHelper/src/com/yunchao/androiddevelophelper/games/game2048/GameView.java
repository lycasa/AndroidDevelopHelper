...
public class GameView extends GridLayout implements OnTouchListener {
	...
	if (gameMatrixHistory.length != 0) {
	    Game2048MainActivity.getGameActivity().setScore(scoreHistory, 0);
	...
	if (blanks.size() > 0) {
	    ...
	    gameMatrix[randomPoint.x][randomPoint.y].setNum(Math.random() > 0.2d ? 1024 : 2048);
	    Game2048MainActivity.getGameActivity().getAnimationLayer().animcreate(gameMatrix[randomPoint.x][randomPoint.y]);
	}
    }
	...
	case MotionEvent.ACTION_UP:
		...
		addRandomNum();
		// 修改显示分数
		Game2048MainActivity.getGameActivity().setScore(Config.Scroe, 0);
	    }
